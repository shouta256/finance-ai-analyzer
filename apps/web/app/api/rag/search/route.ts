...
import { ragSearchResponseSchema } from "@/src/lib/schemas";
...
function mapError(error: unknown): NextResponse {
  ...
  if (process.env.NODE_ENV !== "test") console.error("[api/rag/search] proxy error", { status, code, message, traceId });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const body = searchRequestSchema.parse(raw);
    const result = await ledgerFetch("/rag/search", {
      method: "POST",
