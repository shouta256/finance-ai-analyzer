...
export default function AuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (!API_BASE) {
      setStatus("error");
      setMessage("API base URL is not configured. Please contact support.");
      return;
    }
    ...
    if (env && env !== "prod" && env !== "production") {
      setStatus("error");
      setMessage("API access is disabled in this environment.");
      return;
    }
    ...
    if (!code) {
      setStatus("error");
      setMessage("Missing authorization code. Please log in again.");
      ...
      try {
        ...
        const message =
          error instanceof Error && error.message
            ? error.message
            : "Failed to authorize. Please try signing in again.";
        setMessage(message);
      }
    }
